...
public class StockData {
	...
	private String buildTemplateSubSummary(
				double close,
				double sma200, double sma220,
		        double sma2, double sma3,
		        double sma5, double sma10) {
	...
	*/
	...
	public String processAnalysis( String symbol, Map<String, Object> response) {
		...
		*/
		...
		String template_sub = buildTemplateSubSummary(close, sma200, sma220, sma2, sma3, sma5, sma10);

		String template_above = metaDataStr + "\n\n"
				+ String.format("%s: %s 종가(%.2f)가 200일선(%.2f) **위에(탑승신호)** 있습니다.", today, symbol, close, sma200)
				+ template_sub;
		String template_below = metaDataStr + "\n\n"
				+ String.format("%s: %s 종가(%.2f)가 200일선(%.2f) **아래에(하차신호)** 있습니다.", today, symbol, close, sma200)
				+ template_sub;
		String template_eqaul = metaDataStr + "\n\n"
				+ String.format("%s: %s 종가(%.2f)가 200일선(%.2f) **동일** 합니다.", today, symbol, close, sma200)
				+ template_sub;
		...
		// 주기적으로 api 체크
		if (today.substring(8).contains("10") || today.substring(8).contains("19")
